...
void UartWriteTask(void const * argument);
/* USER CODE BEGIN PFP */
...
int main(void)
{
  ...
  /* Create the queue */
  osMessageQDef(uartQueue, 100, uint8_t);
  uartQueueHandle = osMessageCreate(osMessageQ(uartQueue), NULL);

  /* Create the tasks */
  osThreadDef(uartReadTask, UartReadTask, osPriorityNormal, 0, 128);
  uartReadTaskHandle = osThreadCreate(osThread(uartReadTask), NULL);

  osThreadDef(uartWriteTask, UartWriteTask, osPriorityNormal, 0, 128);
